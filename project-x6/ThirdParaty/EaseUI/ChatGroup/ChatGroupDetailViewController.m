...
{
    [super viewDidLoad];
    // Do any additional setup after loading the view.g3_a
...
- (UIButton *)clearButton
{
    if (_clearButton == nil) {
        ...
        [_clearButton setBackgroundColor:Mycolor];
    }
...
- (UIButton *)dissolveButton
{
    if (_dissolveButton == nil) {
        ...
        [_dissolveButton setBackgroundColor:Mycolor];
    }
...
- (UIButton *)exitButton
{
    if (_exitButton == nil) {
        ...
        [_exitButton setBackgroundColor:Mycolor];
    }
...
#warning 增加群成员将昵称改为环信ID

        NSUserDefaults *userdefaults = [NSUserDefaults standardUserDefaults];
        NSArray *contactList = [userdefaults objectForKey:X6_Contactlist];
        NSMutableArray *source = [NSMutableArray array];
        for (NSDictionary *dic in contactList) {
            for (NSString *buddy in selectedSources) {
                if ([[dic valueForKey:@"nickname"] isEqualToString:buddy]) {
                    [source addObject:[dic valueForKey:@"username"]];
                }
            }
        }
    ...
    for (i = 0; i < row; i++) {
        for (j = 0; j < kColOfRow; j++) {
            ...
            if (index < [self.dataSource count]) {
                ...
                contactView.image = [UIImage imageNamed:@"iocn-moren-n"];
...
- (void)addContact:(id)sender
{
    NSUserDefaults *userdefaults = [NSUserDefaults standardUserDefaults];
    NSArray *contactList = [userdefaults objectForKey:X6_Contactlist];
    NSMutableArray *beenGroupList = [NSMutableArray array];
    for (NSString *easeID in _chatGroup.occupants) {
        for (NSDictionary *dic in contactList) {
            if ([easeID isEqualToString:[dic valueForKey:@"username"]]) {
                [beenGroupList addObject:[dic valueForKey:@"nickname"]];
            }
        }
    }
    ContactSelectionViewController *selectionController = [[ContactSelectionViewController alloc] initWithBlockSelectedUsernames:beenGroupList];
