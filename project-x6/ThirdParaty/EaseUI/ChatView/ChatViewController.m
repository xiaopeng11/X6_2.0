...
- (void)_setupBarButtonItem
{
    ...
    if (self.conversation.conversationType == eConversationTypeChat) {
        UIButton *clearButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 40, 42)];
        [clearButton setImage:[UIImage imageNamed:@"l2_a"] forState:UIControlStateNormal];
        [clearButton setImage:[UIImage imageNamed:@"l2_a1"] forState:UIControlStateNormal];
        [clearButton addTarget:self action:@selector(deleteAllMessages:) forControlEvents:UIControlEventTouchUpInside];
        self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:clearButton];
    }
    else{//群聊
        UIButton *detailButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 40, 24)];
        [detailButton setImage:[UIImage imageNamed:@"l2_b"] forState:UIControlStateNormal];
        [detailButton setImage:[UIImage imageNamed:@"l2_b1"] forState:UIControlStateHighlighted];
                           ...
                           modelForMessage:(EMMessage *)message
{
    ...
    model.avatarImage = [UIImage imageNamed:@"pho-moren"];
    
#warning 环信聊天控制器的头像地址--发送消息时网络请求获取头像
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        NSUserDefaults *userdefaults = [NSUserDefaults standardUserDefaults];
        NSArray *contactList = [userdefaults objectForKey:X6_Contactlist];
        for (NSDictionary *dic in contactList) {
            if ([[dic valueForKey:@"username"] isEqualToString:message.from]) {
